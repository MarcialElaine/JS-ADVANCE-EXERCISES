...
btnSave.addEventListener('click', function() {
    ...
    localStorage.setItem('tblRecords', tblRecords.innerHTML);
    alert('Records saved to local storage!');
});

sortBySelect.addEventListener('change', function() {
    // Sort table by selected option
    sortTable(sortBySelect.value, sortDirectionSelect.value);
});
...
function sortTable(sortBy, sortDirection) {
    const rows = Array.from(tblRecords.rows).slice(1); // Exclude header row

    rows.sort(function(a, b) {
        const aValue = a.cells[sortBy === 'FirstName' ? 0 : 2].textContent.trim();
        const bValue = b.cells[sortBy === 'FirstName' ? 0 : 2].textContent.trim();
